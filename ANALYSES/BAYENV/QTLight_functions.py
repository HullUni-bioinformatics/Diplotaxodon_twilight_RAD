...
def smoothing(dictionary, window_size, factor, to_smooth, chromos=[], significance_level=0.05, sigma_factor=3, bootstrap_reps=0, verbose=0):
    """
...
def write_stats(dictionary, out_columns, factor, prefix, chromos=[]):

    if not chromos:
        for key in dictionary['SNPids']:
            chromos.append(dictionary['SNPids'][key]['chrom'].split("\t")[0])
            
        chromos = sorted(list(set(chromos)))
    ...
    for SNPid in dictionary['global'][factor].keys():
        ...
        if outstring.split("\t")[0] in chromos:
            for column in out_columns:
#                print column
#                print glob[fac][SNPid][column]
                if column in dictionary['global'][factor][SNPid]:
                    temp_list.append(str(dictionary['global'][factor][SNPid][column]))
            if len(temp_list) == len(out_columns):
