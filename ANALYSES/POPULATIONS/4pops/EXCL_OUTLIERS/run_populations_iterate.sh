...
min_pop=4
blacklist=/media/chrishah/STORAGE/RAD/popgen/Fst-outlier/Diplotaxodon/compare/consensus_candidates_list.txt
...
do
	for r in $(seq 0.5 0.1 1)
	do
	        mkdir r_$r-p_$p
                cd r_$r-p_$p
                date
                cmd="populations -b 1 -P ../stacks_data -M $pop -t $threads -r $r -p $p -m $min_depth --fstats -B $blacklist --fasta --genomic --vcf --structure --plink --phylip"
                echo -e "\n$cmd\n"
                $cmd
