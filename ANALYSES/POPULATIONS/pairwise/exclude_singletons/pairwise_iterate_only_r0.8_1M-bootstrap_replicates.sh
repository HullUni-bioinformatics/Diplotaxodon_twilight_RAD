...
#	echo -e "populationmap is $pop"
	...
	do
		...
		do
			...
			echo -e "create whitelist for bootstrapping - only these tags will be bootstrapped"
			zcat ../r_0.8-p_2-w50kb/batch_1.fst_$population1-$population2.tsv.gz | grep "#" -v |perl -ne 'chomp;  @a=split("\t"); if ($a[-2]<0.001){print "$a[1]\n"}' |sort -n | uniq > bootstrap_whitelist.txt

			cmd="populations -b 1 -P ../../stacks_data -W whitelist.txt -M ../populationmap -t $threads -r $r -p $p -m $min_depth --window_size 50000 --fstats -k --fasta --genomic --vcf --structure --plink --phylip --write_single_snp --bootstrap --bootstrap_fst --bootstrap_reps $bootstrap_reps --bootstrap_wl bootstrap_whitelist.txt"
			echo -e "\n$cmd\n"
			$cmd
